...
Original file is located at
    https://colab.research.google.com/drive/1xrSchmVgGhl-S6o_U1Eap_HCmsl1DBDI
...
"""#### **Convert Data Type and Convert Text to Lowercase**

Data dalam setiap baris memiliki format penulisan yang tidak konsisten. Kode berikut, bertujuan untuk mengonversi semua nilai dalam kolom kategorikal menjadi huruf kecil (lowercase) guna menjaga konsistensi data dan menghindari perbedaan akibat huruf kapital.
"""
...
Mengonversi tipe data yang tidak sesuai dalam setiap kolom di dataset, langkah ini penting dalam preprocessing agar data lebih efisien, akurat, dan siap untuk dianalisis atau dimodelkan.
"""
...
menangani missing values agar tidak mengganggu proses analisis dan modeling.
"""
...
"""**Insights:**
* Model yang digunakan yaitu XGBoost (XGBClassifier) dan LightGBM (LGBMClassifier) dibuat dengan parameter default.  
* Hanya parameter `random_state` yang diberikan nilai `42` untuk memastikan hasil yang dapat direproduksi.  
* Pada LGBMClassifier, parameter `verbose=-1` digunakan untuk menonaktifkan output log selama proses pelatihan.
...
for model in models:
    ...
    print(f'Customer Prediction: {y_test_pred}')

    # konversi prediksi ke dalam teks (dengan NumPy)
    churn_labels = np.where(y_test_pred == 1, 'churn', 'not churn')
    print(churn_labels)
    print()
...
* LGBMClassifier memiliki mean score yang sedikit lebih tinggi, yaitu `92.01%`, dibandingkan dengan XGBClassifier yang mencapai `91.99%`. Meskipun selisihnya hanya `0.02%`, perbedaan ini cukup berarti karena dievaluasi menggunakan 5 fold cross validation. Hal ini menunjukkan bahwa LGBMClassifier secara konsisten memberikan performa yang lebih baik dibandingkan XGBClassifier.
...
"""**Insights**:
* Meskipun ROC AUC score dari LGBMClassifier (82.20%) sedikit lebih tinggi dibandingkan XGBClassifier (80.99%), perbedaan ini sangat kecil, menunjukkan bahwa keduanya memiliki performa yang hampir sama. Namun, dalam hal confusion matrix, LGBMClassifier lebih unggul dalam memprediksi churn (kelompok 1), dengan 233 pelanggan churn yang diprediksi dengan benar, dibandingkan 220 pada model XGBoost. Ini berarti LGBMClassifier sedikit lebih efektif dalam mendeteksi pelanggan dengan kecenderungan churn.
